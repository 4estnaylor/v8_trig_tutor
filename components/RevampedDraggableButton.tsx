  ...
  return (
    <Wrapper>
      <Draggable>
        ...
        >
          <Outer>
            <Middle size={size} colorString={cl.getHSL(color)}>
              {/* <Inner></Inner> */}
            </Middle>
          </Outer>
        </div>
...
`;
const Middle = styled.div<{ size: number; colorString: string }>`
  border-radius: 50%;
  height: ${(p) => p.size * 20}px;
  width: ${(p) => p.size * 20}px;
  background-color: ${(props) => props.colorString};
  box-shadow: ${cl.getHSLA(cl.green, 1)} 2px 2px 6px;
  border: none;
  display: flex;
  align-items: center;
  justify-content: center;
