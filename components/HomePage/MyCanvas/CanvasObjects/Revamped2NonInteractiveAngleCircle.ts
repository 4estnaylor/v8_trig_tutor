...
class Revamped2NonInteractiveAngleCircle {
  ...
  constructor(
    ...
    this.center = { x: 195, y: this.canvas.height / 2 };
  ...
  drawFullDivisions = () =>
    ...
    // revolutions: number = 0
    {
      ...
      let totalDashes = this.angleInfoRef.current.divisions;
      ...
      let dashColor =
        this.angleInfoRef.current.angle > 0
          ? cl.getHSL(cl.blue)
          : cl.getHSL(cl.red);
      let angleIncrement = Tau / this.angleInfoRef.current.divisions;

      let minLength = 5;

      if (this.angleInfoRef.current.angle < 0) {
        angleIncrement *= -1;
      }
  ...
  drawLeadLine = () => {
    ...
    let angleOffset = this.angleInfoRef.current.angleOffset;
    let leadCosValue = Math.cos(
      this.angleInfoRef.current.angle + this.angleInfoRef.current.angleOffset
    );
    let leadSinValue = Math.sin(
      this.angleInfoRef.current.angle + this.angleInfoRef.current.angleOffset
    );
    let leadX = this.center.x + leadCosValue * this.radius;
    let leadY = this.center.y - leadSinValue * this.radius;
    let lead = { x: leadX, y: leadY };
    let angle = this.angleInfoRef.current.angle;
...
export default Revamped2NonInteractiveAngleCircle;
